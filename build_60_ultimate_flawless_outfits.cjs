...
async function processPureBfsCutout(srcPath, destPath) {
  ...
  // Sample outer border colors for dynamic background matching
  const borderColors = [];
  const addSample = (x, y) => {
    const idx = (y * width + x) * 4;
    borderColors.push([pixels[idx], pixels[idx + 1], pixels[idx + 2], pixels[idx + 3]]);
  };
  ...
  const isBackgroundPixel = (x, y) => {
    ...
    // Outer background is white, near-white, or off-white studio backdrop
    if (minVal > 175 && diff < 55) return true;
    if (r > 195 && g > 195 && b > 195) return true;
    if (maxVal < 60 && diff < 30) return true; // black frame padding

    for (const [br, bg, bb, ba] of borderColors) {
      if (ba < 20) continue;
      const distSq = (r - br) ** 2 + (g - bg) ** 2 + (b - bb) ** 2;
      if (distSq < 4500) return true;
    }

    return false;
  };

  // Connected BFS flood fill ONLY from outer edges into character silhouette
  while (head < tail) {
    const x = queueX[head];
    const y = queueY[head];
    head++;
  ...
  // Apply alpha zero to outer background, soft edge feathering on boundary
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const idx1d = y * width + x;
      const pIdx = idx1d * 4;
      ...
      if (isBg[idx1d]) {
        ...
        // Soft feather edge pixels adjacent to background
        let bgNeighbors = 0;
        ...
        if (bgNeighbors > 0) {
          ...
          pixels[pIdx + 3] = Math.round(alpha * (1 - bgNeighbors / 14));
        }
      }
    }
  }
...
async function buildAll60OutfitsNoHoles() {
  console.log('🚀 Processing ALL 60 Unique Outfits with PURE BFS (ZERO HOLES INSIDE)...');
  ...
  for (let mapIdx = 1; mapIdx <= 15; mapIdx++) {
    ...
    for (const rarity of rarities) {
      ...
      if (fs.existsSync(sourcePath)) {
        try {
          await processPureBfsCutout(sourcePath, outPath);
          console.log(`✅ Pure BFS Cutout [${outfitId}] -> ${fileName}`);
        } catch (err) {
  ...
  if (fs.existsSync(defaultPinkSource)) {
    await processPureBfsCutout(defaultPinkSource, path.join(OUTFIT_DIR, 'default_pink.png'));
    console.log('✅ default_pink.png');
  }

  console.log('🎉 ALL 60 UNIQUE OUTFITS PROCESSED WITH ZERO HOLES INSIDE!');
}

buildAll60OutfitsNoHoles();
