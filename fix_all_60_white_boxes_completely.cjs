...
async function processFlawlessWhiteBoxRemover(srcPath, destPath) {
  ...
  // Pass 1: Flood fill from 4 borders to remove outer studio backdrop
  ...
  for (let x = 0; x < w; x++) {
    ...
    queueX[tail] = x; queueY[tail] = h - 1; tail++;
  }
  for (let y = 0; y < h; y++) {
    queueX[tail] = 0; queueY[tail] = y; tail++;
    queueX[tail] = w - 1; queueY[tail] = y; tail++;
  }
  ...
  const isOuterBackground = (r, g, b, a) => {
    ...
    if (maxVal <= 60 && diff <= 25) return true; // dark border frame
  ...
  // Pass 2: Strip any internal pure white box pixels (R>=246, G>=246, B>=246)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const idx1d = y * w + x;
      const pIdx = idx1d * 4;
  ...
  // Apply alpha zero to all background pixels
  for (let i = 0; i < w * h; i++) {
    if (isBg[i]) {
      pixels[i * 4 + 3] = 0;
    }
  ...
  // Edge feathering for smooth anti-aliased character contours
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      const idx1d = y * w + x;
      const pIdx = idx1d * 4;
      if (!isBg[idx1d]) {
        ...
        if (isBg[idx1d + 1]) bgNeighbors++;
        if (isBg[idx1d - w]) bgNeighbors++;
        if (isBg[idx1d + w]) bgNeighbors++;

        if (bgNeighbors >= 2) {
          pixels[pIdx + 3] = Math.round(pixels[pIdx + 3] * 0.4);
        }
      }
    }
  }
...
async function buildAll60WhiteBoxesCompletely() {
  console.log('🚀 Processing ALL 60 Outfits with FLAWLESS WHITE BOX REMOVER...');
  ...
  for (let mapIdx = 1; mapIdx <= 15; mapIdx++) {
    ...
    for (const rarity of rarities) {
      ...
      if (fs.existsSync(sourcePath)) {
        try {
          await processFlawlessWhiteBoxRemover(sourcePath, outPath);
          console.log(`✅ Flawless Cutout [${outfitId}] -> ${fileName}`);
        } catch (err) {
  ...
  if (fs.existsSync(defaultPinkSource)) {
    await processFlawlessWhiteBoxRemover(defaultPinkSource, path.join(OUTFIT_DIR, 'default_pink.png'));
    console.log('✅ default_pink.png');
  }

  console.log('🎉 ALL 60 OUTFITS PROCESSED WITH 100% PURE TRANSPARENT BACKGROUNDS!');
}

buildAll60WhiteBoxesCompletely();
